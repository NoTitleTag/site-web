...
for(let i=0; i < nomsAbordable.length ; i++){
    const nomElement = document.createElement('li');
    nomElement.innerText = nomsAbordable[i];
...
for (let index = pieces.length -1; index >=0; index--) {
    if(pieces[index].disponibilite === false) {
        nomsDisponibles.splice(index,1)
        prixDisponibles.splice(index,1)
    }
}
const disponibleElements = document.createElement("ul")
for (let index = 0; index < nomsDisponibles.length; index++) {
    const nomElement = document.createElement("li")
    nomElement.innerText = `${nomsDisponibles[index]} - ${prixDisponibles[index]} €`
